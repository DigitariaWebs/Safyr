...
export default function LoginScreen() {
  ...
  const canSubmit = useMemo(() => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()) && password.length >= 1, [
    email,
    password,
  ]);
      ...
      >
        <View className="flex-1 justify-center">
          ...
          <Card className="gap-4">
            <View>
              <Text className="mb-2 text-sm font-medium text-foreground" style={{ fontFamily: getBodyFont("500") }}>Email</Text>
              <Input
                value={email}
            ...
            <View>
              <Text className="mb-2 text-sm font-medium text-foreground" style={{ fontFamily: getBodyFont("500") }}>Mot de passe</Text>
              <Input
                value={password}
            ...
            <Text className="text-xs text-muted-foreground" style={{ fontFamily: getBodyFont("400") }}>
